...
(function(){
  ...
  const NOLO_MODEL_MODE = "mini";
  const NOLO_SYSTEM_MODE = "serio_educativo_riesgo";
  ...
  window.loadNoloHistory=async function(force){ const user=getUser(); if(force)clearBoxes(); if(!user?.id||typeof supabaseClient==="undefined"){addBubble("ai","Estoy listo. Inicia sesión para guardar memoria y usar el límite diario."); await refreshUsage(); return;} try{const {data,error}=await supabaseClient.from("nolo_messages").select("role,message,created_at").eq("user_id",user.id).order("created_at",{ascending:true}).limit(80); if(error)throw error; clearBoxes(); if(!data||!data.length){ const profile=getPerformanceProfile(); addBubble("ai",`Hola ${profile.studentName} 👋 Soy Nolo IA. Estoy en modo serio. Puedo leer tu perfil, consistencia, control de riesgo, noticias, emociones y disciplina. ¿Quieres un briefing antes de operar o revisión de tu perfil?`); } else data.forEach(m=>addBubble(m.role==="user"?"user":"ai",m.message,m.created_at)); await refreshUsage();}catch(err){console.error("loadNoloHistory",err); addBubble("ai","No pude cargar la memoria ahora mismo. Verifica la tabla nolo_messages en Supabase."); await refreshUsage();} };
  ...
  window.sendNoloMessage=async function(){
    ...
    try{const res=await fetch(NOLO_AI_FUNCTION_URL,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({action:"chat",userId:user.id,userName:user.full_name||user.name||"Estudiante",plan:user.plan||"VIP Regular",message:text,newsContext:getNewsContext(),performanceProfile:getPerformanceProfile(),dailyLimit:DAILY_LIMIT,modelMode:NOLO_MODEL_MODE,systemMode:NOLO_SYSTEM_MODE,portalBrand:"OLON Society Academy",assistantName:"Nolo IA Mini",safetyRules:["educativo", "sin prometer ganancias", "no señales garantizadas", "enfocado en riesgo y disciplina"]})}); const data=await res.json().catch(()=>({})); setTyping(false); if(res.status===429){setStatus(data.remaining??0,data.used??DAILY_LIMIT); addBubble("ai",`Llegaste al límite diario de ${DAILY_LIMIT} mensajes. Usa el botón Hablar con admin si necesitas ayuda.`); playNoloSound(); return;} if(!res.ok)throw new Error(data.error||"Error conectando Nolo IA"); addBubble("ai",data.reply||"No recibí respuesta de Nolo IA."); playNoloSound(); setStatus(data.remaining,data.used); resetNoloIdleTimer();}
  ...
  function ensureFloatingNolo(){document.getElementById("aiMentorBtn")?.remove(); document.getElementById("aiMentorPanel")?.remove(); if(document.getElementById("noloFloatingBtn"))return; const btn=document.createElement("button"); btn.id="noloFloatingBtn"; btn.type="button"; btn.innerHTML="🤖<span>NOLO IA</span>"; btn.onclick=window.toggleNoloFloatingChat; document.body.appendChild(btn); const panel=document.createElement("div"); panel.id="noloFloatingPanel"; panel.className="noloFloatPanel hidden"; panel.innerHTML=`<div class="noloFloatHeader"><div><b>🤖 Nolo IA Mini</b><small id="noloFloatStatus">Verificando límite diario...</small></div><button type="button" onclick="toggleNoloFloatingChat()">✕</button></div><div id="noloFloatMessages" class="noloFloatMessages"></div><div class="noloFloatQuick"><button type="button" onclick="askNoloQuick('Lee mi perfil y dame 3 mejoras concretas de consistencia, control de riesgo y disciplina')">Mi perfil</button><button type="button" onclick="askNoloQuick('Analiza noticias actuales y separa impacto alto, medio y bajo')">Noticias</button><button type="button" onclick="askNoloQuick('Cómo afecta esto al oro XAUUSD')">Oro</button><button type="button" onclick="openNoloAdminContact()">Hablar con admin</button></div><div id="noloAdminBox" class="noloAdminBox hidden"><textarea id="noloAdminInput" placeholder="Escribe tu duda para el admin..."></textarea><button class="btn" type="button" onclick="sendNoloAdminMessage()">Enviar al portal admin</button></div><div class="noloFloatInput"><input id="noloFloatInput" placeholder="Escribe a Nolo IA..." onkeydown="if(event.key==='Enter') sendNoloMessage()"><button id="noloFloatSendBtn" type="button" onclick="sendNoloMessage()">Enviar</button></div>`; document.body.appendChild(panel); window.loadNoloHistory(false); refreshUsage();}
  ...
  function hookShowPage(){const old=window.showPage; window.showPage=function(id,btn){const r=typeof old==="function"?old(id,btn):undefined; if(id==="noloAIPage"){const title=document.getElementById("pageTitle"),sub=document.getElementById("pageSubtitle"); if(title)title.textContent="Nolo IA Mini Pro"; if(sub)sub.textContent="Asistente serio con perfil, memoria, noticias, control de riesgo y límite diario."; setTimeout(()=>window.loadNoloHistory(true),80);} return r;};}
...
/* =====================================================
   OLON ENTRETENIMIENTO: RULETA + FLAPPY JS
   ===================================================== */
(function(){
  ...
  function initEntertainment(){
    if(entReady || !$('entWheelCanvas') || !$('entGameCanvas')) return;
    entReady = true;

    const wheelCanvas = $('entWheelCanvas');
    const wctx = wheelCanvas.getContext('2d');
    const spinBtn = $('entSpinBtn');
    const prizesInput = $('entPrizesInput');
    const savePrizesBtn = $('entSavePrizesBtn');
    const winnerText = $('entWinnerText');
    const historyList = $('entHistoryList');
    const clearHistoryBtn = $('entClearHistoryBtn');
    const colors = ['#f6c453','#22d3ee','#a855f7','#22c55e','#fb7185','#60a5fa','#f97316','#14b8a6','#e879f9','#84cc16'];
    let prizes = JSON.parse(localStorage.getItem('olon_ent_prizes') || 'null') || ['Premium 15 días','Mentoría 1 a 1','Indicador gratis 7 días','Descuento 50%','Acceso VIP','Premio sorpresa','Clase privada','Entrada al grupo'];
    let history = JSON.parse(localStorage.getItem('olon_ent_history') || '[]');
    let angle = 0;
    let spinning = false;
    prizesInput.value = prizes.join('\n');

    window.entDrawWheel = function(){
      if(!wctx || !prizes.length) return;
      const cx = wheelCanvas.width/2, cy = wheelCanvas.height/2, radius = Math.min(cx,cy)-26;
      const slice = Math.PI*2/prizes.length;
      wctx.clearRect(0,0,wheelCanvas.width,wheelCanvas.height);
      wctx.save(); wctx.translate(cx,cy); wctx.rotate(angle);
      for(let i=0;i<prizes.length;i++){
        const start=i*slice, end=start+slice;
        wctx.beginPath(); wctx.moveTo(0,0); wctx.arc(0,0,radius,start,end); wctx.closePath();
        wctx.fillStyle=colors[i%colors.length]; wctx.fill();
        wctx.strokeStyle='rgba(2,6,23,.42)'; wctx.lineWidth=5; wctx.stroke();
        wctx.save(); wctx.rotate(start+slice/2); wctx.textAlign='right'; wctx.textBaseline='middle';
        wctx.fillStyle='#07111f'; wctx.font='900 24px Inter, Arial, sans-serif';
        let label=prizes[i]; if(label.length>18) label=label.slice(0,17)+'…';
        wctx.fillText(label, radius-30, 0); wctx.restore();
      }
      wctx.beginPath(); wctx.arc(0,0,radius,0,Math.PI*2); wctx.strokeStyle='rgba(255,255,255,.42)'; wctx.lineWidth=10; wctx.stroke();
      wctx.restore();
    };
    ...
    function renderHistory(){
      ...
      if(!history.length){historyList.innerHTML='<div class="entHistoryItem"><span>Sin historial</span><span>—</span></div>';return;}
      history.forEach(function(item){
        const row=document.createElement('div'); row.className='entHistoryItem';
        row.innerHTML='<span>'+String(item.prize).replace(/[<>]/g,'')+'</span><span>'+String(item.time).replace(/[<>]/g,'')+'</span>';
    ...
    function pickWinner(){
      ...
      const index=Math.floor(pointerAngle/(Math.PI*2/prizes.length))%prizes.length;
      return prizes[index];
    }

    savePrizesBtn?.addEventListener('click', function(){
      const list=prizesInput.value.split('\n').map(x=>x.trim()).filter(Boolean);
      if(list.length<2){entToast('Pon mínimo 2 premios.');return;}
      prizes=list; localStorage.setItem('olon_ent_prizes',JSON.stringify(prizes));
      window.entDrawWheel(); entToast('Premios guardados.');
    });

    clearHistoryBtn?.addEventListener('click', function(){
      history=[]; localStorage.removeItem('olon_ent_history'); renderHistory(); entToast('Historial borrado.');
    });

    spinBtn?.addEventListener('click', function(){
      if(spinning) return;
      spinning=true; spinBtn.disabled=true;
      const start=angle, target=angle+(6+Math.random()*5)*Math.PI*2+Math.random()*Math.PI*2;
      const duration=5200, startTime=performance.now();
      function ease(t){return 1-Math.pow(1-t,3)}
      function anim(now){
        const t=Math.min((now-startTime)/duration,1);
        angle=start+(target-start)*ease(t); window.entDrawWheel();
        if(t<1) requestAnimationFrame(anim); else{
          spinning=false; spinBtn.disabled=false; angle=angle%(Math.PI*2);
          const winner=pickWinner(); winnerText.textContent='🏆 '+winner;
          history.unshift({prize:winner,time:new Date().toLocaleString('es-PR',{hour:'numeric',minute:'2-digit',month:'short',day:'numeric'})});
          history=history.slice(0,20); localStorage.setItem('olon_ent_history',JSON.stringify(history)); renderHistory(); entToast('Ganador: '+winner);
        }
      }
      requestAnimationFrame(anim);
    });

    window.entDrawWheel(); renderHistory();
    ...
    const game = {running:false,over:false,score:0,best:Number(localStorage.getItem('olon_ent_flappy_best')||0),frame:0,bird:{x:92,y:250,w:72,h:34,r:14,vy:0},pipes:[],clouds:[],groundX:0,gravity:(window.innerWidth<=700?.23:.16),jump:(window.innerWidth<=700?-5.25:-4.65),pipeSpeed:(window.innerWidth<=700?2.05:1.35),pipeGap:(window.innerWidth<=700?175:185),pipeEvery:(window.innerWidth<=700?112:138)};
  ...
  setTimeout(function(){
    ...
    if(typeof old === 'function' && !window.__entertainmentShowPageHooked){
      window.showPage = function(id, btn){
        const r = old.apply(this, arguments);
        if(id === 'entretenimiento'){
          initEntertainment();
      ...
      window.__entertainmentShowPageHooked = true;
    }
  }, 900);
})();